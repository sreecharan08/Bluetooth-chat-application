...
from PyQt6.QtWidgets import (
    ...
    QListWidgetItem, QGroupBox, QToolButton
)
from PyQt6.QtCore import Qt

import storage
from transport import BluetoothWorker, winrt_bluetooth_available
from discovery import DiscoveryWorker, PairWorker
from protocol import Envelope, MessageType, make_text_message
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # --- Nearby devices ---
        discovery_box = QGroupBox("Nearby BTChat devices")
        ...
        self.advanced_toggle.setText("Advanced: connect by MAC address")
        ...
        advanced_layout.addWidget(self.mac_input, stretch=1)
    ...
    def _on_devices_found(self, devices: list):
        ...
        self.device_list.clear()
        if not devices:
            placeholder = QListWidgetItem("No BTChat devices found nearby. Make sure it's open on the other PC.")
            placeholder.setFlags(Qt.ItemFlag.NoItemFlags)
            self.device_list.addItem(placeholder)
            return
        for d in devices:
            label = d["name"] + ("  [paired]" if d["is_paired"] else "  [unpaired]")
    ...
    def _on_connected(self, peer_label: str):
        ...
        self.chat.set_status(f"Connected to {peer_label}", connected=True)
